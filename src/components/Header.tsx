...
export default function Header({ onNavigate, onSearch }: { onNavigate: (key: string) => void; onSearch: (q: string) => void }) {
    ...
    <div>
      ...
      <div className="win95-outset flex items-center justify-between flex-wrap gap-1 px-2 py-1">
        <div className="flex gap-[2px] flex-wrap">
          {([
            ...
            <span
              ...
              className={`text-[#CC0000] underline font-mono text-[12px] px-1 py-[1px] ${key ? "cursor-pointer opacity-100" : "cursor-default opacity-50"}`}
            >{label}</span>
          ))}
        </div>
        <div className="flex items-center gap-1">
          ...
          />
          <button
            onClick={() => { if (q.trim()) { onSearch(q); setQ(""); } }}
